...
@click.option(
    "--category", "-c", prompt=True, help="Category of the expense", default="Other"
)
@click.option(
    "--date", "-d", prompt=True, help="Date of the expense", default=date.today()
)
@click.option(
    "--description",
    "-D",
...
@click.option(
    ...
    "-f",
    type=click.Choice(
        ["amount", "category", "date", "description", "amount__range", "date__range"]
    ),
    default=[],
    multiple=True,
)
@click.option(
    ...
    "-s",
    type=click.Choice(
        ["date", "amount", "category", "description", "date__asc", "amount__asc"]
    ),
    default=["date"],
    multiple=True,
)
        ...
        with session.begin():
            ...
            while True:
                value = click.prompt(
                    ...
                    type=click.Choice(["amount", "category", "date", "description"]),
                )
                if value == "amount":
...
def paginate_query(
    query, offset: int = 0, page: int = 1, page_size: int = 5
) -> models.Expense:
    ...
    if answer == "load more":
        return paginate_query(
            query, offset=offset + page_size, page=page + 1, page_size=page_size
        )
