...
def main():
    ...
    df = pd.read_sql(
        sql="SELECT id, category, date, amount FROM expense", con=engine, index_col="id"
    )
    df = df.pivot_table(
        index="date", columns="category", values="amount", aggfunc="sum", fill_value=0
    )

    # Fill empty dates
    monday = date.today() - timedelta(days=date.today().weekday())
    daterange = pd.date_range(
        start=monday, periods=7, freq="D", inclusive="both", normalize=True
    )
